...
view_option = st.sidebar.radio("Choose what to display", ("Tables", "Staff Data", "Combined Staff Table", "Combined RE Data"))
...
elif view_option == "Combined RE Data":
    db_connection = connect_to_db()
    cursor = db_connection.cursor()
    ...
    if selected_province:
        ...
        if selected_district:
            ...
            if selected_local_authority:
                ...
                selected_year = st.sidebar.selectbox("Select a Year", year_list)
                if selected_year:
                    # Debugging: Print column names for each table
                    ...
                    st.write("Checking column names for each table...")
                    for table in ["revenues", "expenditures", "local_authority_programs", "program_types"]:
                        st.write(f"Table: {table}")
                        print_column_names(table)

                    # Fetch revenue, expenditure, local authority programs, and program types
                    queries = {
                        "revenues": """
                        ...
                        "local_authority_programs": """
                            SELECT * FROM local_authority_programs
                            WHERE local_authority_id = %s AND year = %s
                        """,
                        "program_types": """
                    ...
                    for key, query in queries.items():
                        try:
                            ...
                            # Get column names dynamically if the query returns data
                            columns = [desc[0] for desc in cursor.description]
                            df = pd.DataFrame(data, columns=columns)
                            combined_data[key] = df
                        except mysql.connector.Error as err:
                            st.write(f"Error fetching data for {key}: {err}")

                    # Combine all data into a single DataFrame
                    if combined_data:
                        combined_df = pd.concat(combined_data.values(), axis=1)
                        # Display the combined DataFrame
                        st.subheader("Combined RE Data")
                        st.write(f"Data for {selected_province} -> {selected_district} -> {selected_local_authority} ({selected_year})")
                        st.dataframe(combined_df)
                    else:
