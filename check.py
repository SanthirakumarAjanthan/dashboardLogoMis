...
# Sidebar to choose between different functionalities using a selectbox
view_option = st.sidebar.selectbox("Choose what to display", 
                                   ("Database_Tables", "Staff_Details", "General_Info",
                                    "Annual_Budgets", "Program_Title", "Detail_of_GNDs", 
                                    "Details_of_Wards", "Actual_budgets", "ER Diagram"))
...
if view_option == "Database_Tables":
    ...
    if db_connection is None:
        ...
        if selected_district:
            ...
            local_authorities = cursor.fetchall()
            la_options = {name: id for id, name in local_authorities}
            selected_la = st.sidebar.selectbox("Select Local Authority", list(la_options.keys()))

        # Year Selection
        cursor.execute(f"SELECT DISTINCT year FROM local_authority_information WHERE local_authority_id = {la_options[selected_la]}")
        years = cursor.fetchall()
        year_options = [year[0] for year in years]
        selected_year = st.sidebar.selectbox("Select Year", year_options)

        # Display the General Information for the selected LA and Year
        if selected_la and selected_year:
            cursor.execute(f"SELECT id, question_id, answer FROM local_authority_information WHERE local_authority_id = {la_options[selected_la]} AND year = {selected_year}")
            results = cursor.fetchall()

            st.subheader(f"General Information - {selected_la} ({selected_year})")
            for id, question_id, answer in results:
                formatted_answer = format_answer(question_id, answer, cursor)
                st.markdown(styled_box("<br>".join(formatted_answer)), unsafe_allow_html=True)
